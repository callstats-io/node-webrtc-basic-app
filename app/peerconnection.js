PeerConnectionChannel = function (to, from, div,
  localStreamParam, onPCInitialized, onPCError) {
  this.div = div;
  this.from = from;
  this.to = to;
  var localStream;
  ...
  this.doCallIfActive = function () {
    if (isCallActive) doCall();
  };

  this.getPeerConnection = function () {
    console.log('getPeerConnection', pc, this.pc);
    return pc;
  };
  ...
  function handleOnSignaliingChange(event) {
    console.log('********* handleOnSignaliingChange', event.target.signalingState);
  }

  function handleOnConnectionChange(event) {
  ...
  function createPeerConnection() {
    try {
      ...
      pc.addEventListener('connectionstatechange', handleOnConnectionChange, false);
      pc.addEventListener('signalingstatechange', handleOnSignaliingChange, false);
  ...
  function handleRemoteStreamAdded(event) {
    ...
    ssrcs.forEach(function (ssrc) {
      window.callStats.associateMstWithUserID(
        pc,
        to,
        "foo",
        ssrc,
    ...
    getElement(tag, function (element) {
      if (element === false || element === null) {
        ...
        remoteVideo.setAttribute(
          "style",
          "-webkit-transition: opacity 2s; opacity: 1; margin-right: 3px;"
        );
